...
class ApiEndpoints {
    constructor(crossExtensionReader) {
    ...
    getAllCharacters(req, res) {
        try {
            ...
            if (!(0, utils_1.isValidId)(extensionId)) {
    ...
    upsertCharacter(req, res) {
        ...
        try {
            ...
            const validation = (0, utils_1.validateRequestBody)(['id'], req.body);
            if (!validation.isValid) {
                console.log(chalk.yellow(MODULE_NAME), 'POST /characters failed: Missing required fields:', (_a = validation.missingFields) === null || _a === void 0 ? void 0 : _a.join(', '));
                res.status(400).json({ error: `Missing required fields: ${(_b = validation.missingFields) === null || _b === void 0 ? void 0 : _b.join(', ')}` });
                return;
            }
    ...
    getAllInstances(req, res) {
        try {
            ...
            if (typeof characterId === 'string' && (0, utils_1.isValidId)(characterId)) {
    ...
    getInstancesByCharacter(req, res) {
        try {
            const extensionId = this.getExtensionIdFromHeader(req);
            if (!extensionId) {
                res.status(400).json({ error: 'Extension ID is required in header: x-extension-id' });
            ...
            if (!(0, utils_1.isValidId)(characterId)) {
                res.status(400).json({ error: 'Valid character ID is required (alphanumeric, hyphens, underscores only)' });
                return;
            }
            const dbManager = this.crossExtensionReader.getDbManager(extensionId);
    ...
    upsertInstance(req, res) {
        ...
        try {
            ...
            // Validate the request body
            const validation = (0, utils_1.validateRequestBody)(['id', 'characterId'], req.body);
            if (!validation.isValid) {
                console.log(chalk.yellow(MODULE_NAME), 'POST /instances failed: Missing required fields:', (_a = validation.missingFields) === null || _a === void 0 ? void 0 : _a.join(', '));
                res.status(400).json({ error: `Missing required fields: ${(_b = validation.missingFields) === null || _b === void 0 ? void 0 : _b.join(', ')}` });
                return;
            }
            const { id, characterId, name } = req.body;
            if (!(0, utils_1.isValidId)(id) || !(0, utils_1.isValidId)(characterId)) {
    ...
    getInstanceDataKey(req, res) {
        try {
            ...
            if (!(0, utils_1.isValidId)(id) || typeof key !== 'string' || key.trim().length === 0) {
    ...
    upsertInstanceData(req, res) {
        ...
        try {
            ...
            // Validate the request body
            const validation = (0, utils_1.validateRequestBody)(['key'], req.body);
            if (!validation.isValid) {
                console.log(chalk.yellow(MODULE_NAME), 'POST /instances/:id/data failed: Missing required fields:', (_a = validation.missingFields) === null || _a === void 0 ? void 0 : _a.join(', '));
                res.status(400).json({ error: `Missing required fields: ${(_b = validation.missingFields) === null || _b === void 0 ? void 0 : _b.join(', ')}` });
                return;
            }
            if (!(0, utils_1.isValidId)(id) || typeof key !== 'string' || key.trim().length === 0) {
    ...
    // Complex operations
    deleteAllInstancesForCharacter(req, res) {
        try {
            const extensionId = this.getExtensionIdFromHeader(req);
            if (!extensionId) {
                res.status(400).json({ error: 'Extension ID is required in header: x-extension-id' });
    ...
    overrideInstanceData(req, res) {
        try {
            ...
            const { id } = req.params;
            const data = req.body;
            if (!(0, utils_1.isValidId)(id) || typeof data !== 'object' || data === null || Array.isArray(data)) {
    ...
    removeInstanceDataKeys(req, res) {
        var _a, _b;
        try {
            ...
            const { keys } = req.body;
            // Validate the request body
            const validation = (0, utils_1.validateRequestBody)(['keys'], req.body);
            if (!validation.isValid) {
                console.log(chalk.yellow(MODULE_NAME), 'PUT /instances/:id/data/remove failed: Missing required fields:', (_a = validation.missingFields) === null || _a === void 0 ? void 0 : _a.join(', '));
                res.status(400).json({ error: `Missing required fields: ${(_b = validation.missingFields) === null || _b === void 0 ? void 0 : _b.join(', ')}` });
                return;
            }
            if (!(0, utils_1.isValidId)(id) || !Array.isArray(keys)) {
    ...
    registerExtension(req, res) {
        ...
        try {
            ...
            // Validate the request body
            const validation = (0, utils_1.validateRequestBody)(['extensionId'], req.body);
            if (!validation.isValid) {
                console.log(chalk.yellow(MODULE_NAME), 'Registration failed: Missing required fields:', (_a = validation.missingFields) === null || _a === void 0 ? void 0 : _a.join(', '));
                res.status(400).json({ error: `Missing required fields: ${(_b = validation.missingFields) === null || _b === void 0 ? void 0 : _b.join(', ')}` });
                return;
            }
            ...
            const dbManager = new DatabaseManager_1.DatabaseManager(dbPath, extensionId);
    ...
    getCrossExtensionCharacter(req, res) {
        try {
            const { extensionId, id } = req.params;
            if (!(0, utils_1.isValidId)(extensionId) || !(0, utils_1.isValidId)(id)) {
    ...
    getCrossExtensionInstance(req, res) {
        try {
            const { extensionId, id } = req.params;
            if (!(0, utils_1.isValidId)(extensionId) || !(0, utils_1.isValidId)(id)) {
    ...
    getCrossExtensionInstanceData(req, res) {
        try {
            const { extensionId, id } = req.params;
            if (!(0, utils_1.isValidId)(extensionId) || !(0, utils_1.isValidId)(id)) {
                res.status(400).json({ error: 'Valid extension ID and instance ID are required (alphanumeric, hyphens, underscores only)' });
    ...
    getCrossExtensionInstanceDataKey(req, res) {
        try {
            const { extensionId, id, key } = req.params;
            if (!(0, utils_1.isValidId)(extensionId) || !(0, utils_1.isValidId)(id) || typeof key !== 'string' || key.trim().length === 0) {
    ...
    getCrossExtensionAllCharacters(req, res) {
        try {
    ...
    getCrossExtensionInstancesByCharacter(req, res) {
        try {
            const { extensionId, characterId } = req.params;
            if (!(0, utils_1.isValidId)(extensionId) || !(0, utils_1.isValidId)(characterId)) {
                res.status(400).json({ error: 'Valid extension ID and character ID are required (alphanumeric, hyphens, underscores only)' });
                return;
            }
